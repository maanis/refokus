...
const Work = () => {
    ...
    return (
        <div className='max-w-screen-lg max-md:max-w-screen-2xl max-lg:min-h-[60vh] max-md:min-h-[70vh]  flex justify-center items-center select-none text-center text-zinc-300 relative py-8 mx-auto'>
            ...
            {images.map((img, index) => img.isActive && <img key={index} src={img.url} className={`absolute z-${img.z} w-72 max-lg:w-56 max-md:w-40 max-sm:w-36 rounded-lg object-cover -translate-x-1/2 -translate-y-1/2`} style={{ top: img.top, left: img.left }} />)}
        </div>
    )
}
